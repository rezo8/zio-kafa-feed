...
class MessageReader(config: KafkaConsumerConfig) {
  ...
  // Process for all partitions, returning ZIO of List[Person] instead of Unit
  ...
  // The ZIO version of the process function that returns ZIO of List[Person]
    ...
    for {
      ...
      personList <- ZIO.attempt {
        ...
        while (messagesProcessed < count) {
          ...
          // Exit early if no records were fetched
          if (records.isEmpty) {
